...
void matrixMulAVX2(double* result, const double* matA, const double* matB, size_t sharedDim, size_t rowsA, size_t colsB) {
    ...
    //обычное умножение матриц ускорение с AVX2 + транспонирование матрицы B
    //сначала B транспонируется в TransposeT, чтобы доступ к памяти был последовательным(кэш дружелюбный)
    //дальше суммирование делается векторно по 4 double за раз(__m256d)
    //Обычно это быстрее обычного умножения.

    std::vector<double> TransposeT(colsB * sharedDim, 0.0);

    if (sharedDim % 4 == 0 && colsB % 4 == 0)
        transposeAVX2(TransposeT.data(), matB, sharedDim, colsB);
    else {
...
void matrixMulAVX2_gather(double* result, const double* matA, const double* matB, size_t sharedDim, size_t rowsA, size_t colsB) {

    //Тут матрица B не транспонируется.
    //Вместо этого элементы столбца B достаются через _mm256_i64gather_pd, который собирает значения по адресам.
    //Обычно gather медленнее транспонирования(но зависит от размеров.
...
void speedtest_avg_openmp(size_t n) {
    ...
    std::ofstream output(base_dir.append("output2.csv"));
    if (!output.is_open())
    {
